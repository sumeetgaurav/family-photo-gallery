...
7. The gallery has a **Log out** button (`signOutVisitor` in `src/app/actions.ts`) that clears only this browser's session cookie — the underlying approval and `approved_devices` row are untouched, so re-submitting the same email on this device re-approves instantly rather than waiting again. Only an admin revoke/deny forces a real re-approval.
8. The gallery also auto-logs-out after **15 minutes of inactivity** (`IdleLogout` in `src/components/IdleLogout.tsx`, mounted in `src/app/gallery/page.tsx`) — a client-side timer reset by mouse/keyboard/scroll/touch activity, calling the same `signOutVisitor` as the manual button on expiry. Same semantics as manual log out: local-only, doesn't revoke the device.
...
- `access_requests`: `id, name, email, status (pending|approved|denied), device_cookie_id, created_at, decided_at, decided_by, visit_count, last_visited_at, last_user_agent` — unique on `lower(email)`; one row per email, reused/reset across resubmissions rather than duplicated. `visit_count`/`last_visited_at`/`last_user_agent` are bumped atomically by the `record_gallery_visit(request_id, user_agent)` SQL function on every `/gallery` render (see `src/app/gallery/page.tsx`); the raw UA is parsed into a friendly label (`describeUserAgent` in `src/lib/user-agent.ts`) only at display time in the admin dashboard, so labeling can improve later without a migration.
- `approved_devices`: `id, access_request_id, token_hash, created_at, revoked_at` — one row per approved browser/device; not unique on `access_request_id`, so one email can have several active devices. The admin dashboard aggregates these back to one row per email though (see Deviations) — `revokeAccess` revokes all of a visitor's active devices at once, not one device at a time.
...
- **Admin dashboard shows one row per visitor (email), not one per device.** Each new browser for an already-approved email mints its own `approved_devices` row (previous point), which would otherwise show as duplicate entries for the same person. `AdminRequestsPage` (`src/app/admin/(dashboard)/page.tsx`) instead queries `access_requests` directly for the "Approved visitors" list — one row per email, showing `visit_count`/`last_visited_at` (formatted in IST via `src/lib/format.ts`) — filtered to requests with at least one active device. `revokeAccess` revokes all of that email's active devices in one action.
